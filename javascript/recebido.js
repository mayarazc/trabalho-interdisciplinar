/*  Nome: Henrique Gutler, Igor Luiz Pereira Butke e Mayara Zanetti Carlini
    ...
    Versão: 1.0
    Descrição: Usado para fazer a tabela específica do time e o relatório, para o trabalho interdisciplinar 
...
function mostrarTime() {
    ...
    if (timeA.indexOf(time) != (-1)) {
        ...
        // cria as th para o título da tabela
        var thTime = document.createElement("th");
        thTime.textContent = "Time";

        var thPontuacao = document.createElement("th");
        thPontuacao.textContent = "PTS";

        var thVitorias = document.createElement("th");
        thVitorias.textContent = "V";

        var thEmpate = document.createElement("th");
        thEmpate.textContent = "E";

        var thDerrotas = document.createElement("th");
        thDerrotas.textContent = "D";

        var thGolsPro = document.createElement("th");
        thGolsPro.textContent = "GP";

        var thGolsContra = document.createElement("th");
        thGolsContra.textContent = "GC";

        var thSaldo = document.createElement("th");
        thSaldo.textContent = "SG";

        // cria os parágrafos para guardar os dados do relatório
        ...
        //Criando o trThead da tabela
        trThead.appendChild(thTime);
        trThead.appendChild(thPontuacao);
        trThead.appendChild(thVitorias);
        trThead.appendChild(thEmpate);
        trThead.appendChild(thDerrotas);
        trThead.appendChild(thGolsPro);
        trThead.appendChild(thGolsContra);
        trThead.appendChild(thSaldo);
        ...
        relatorio.appendChild(trThead);
        relatorio.appendChild(trDados);

    } else {
        titulo.textContent = "Ops time não encontrado"
    }
}

inCorpo.addEventListener("load", mostrarTime());